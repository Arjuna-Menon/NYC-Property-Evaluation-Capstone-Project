...
'proxcode', 'yearbuilt', 'yearalter','income','lotfront','lotdepth'],axis=1)

data["bo_bloc_lot"] = data["borocode"].map(str)  + data["block"].map(str) + data["lot"].map(str)




# drop also block and lot
data=data.drop(['lot','block', 'borocode'], axis=1)


#Scpecify what columns are factors
to_factors = ["cd","schooldist","council","zipcode","policeprct","firecomp",
               "healtharea","sanitboro","sanitsub","zonedist1","spdist1","ltdheight","landuse",
               "irrlotcode","lottype","edesignum","sanitdistrict",
               "healthcenterdistrict", "pfirm15_flag","bo_bloc_lot"]
...
def a_tree(df1, top_predictors_list):
    ...
    reg = RandomForestRegressor(
            ...
            #NEW VALUES
             #n_estimators= 1000,  # should be: 1000
             #min_samples_split= 5, 
             #min_samples_leaf= 2, 
             #max_features ='sqrt', 
             #max_depth =100, 
